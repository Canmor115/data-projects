# 📊 data-projects

> From raw data to insight — analysis, mining and visualization projects

Repositorio de proyectos personales de análisis de datos, minería y visualización. Cada proyecto aborda un problema real con datos públicos y técnicas aplicadas.

---

## 🗂️ Proyectos

### 🇪🇺 [Trabajo Autónomo en la UE (2010–2024)](./Trabajo_autonomo_UE_2010_2024.ipynb)
**Python · Pandas · Folium · Seaborn · API Eurostat**

Análisis exploratorio de la evolución del trabajo autónomo en Europa. Incluye mapas interactivos por región NUTS 2, análisis por género y nivel educativo, y correlación entre desempleo y emprendimiento. Conclusión central: las mujeres con formación superior emprenden como respuesta al desempleo — emprendimiento por necesidad.

---

### 🏦 [Predicción de Riesgo Crediticio](./credit-risk-prediction/)
**R · C5.0 · Random Forest · XGBoost · SHAP · ALE**

Clasificación de solicitantes de crédito (dataset German Credit) con árboles de decisión y modelos ensemble. Evaluación bajo matriz de costes asimétrica (FN=5, FP=1) y análisis de explicabilidad con Feature Importance, ALE y valores SHAP. El perfil de mayor riesgo identificado: cliente con saldo negativo que solicita préstamo a largo plazo.

🔗 [Ver análisis completo →](https://canmor115.github.io/data-projects/credit-risk-prediction/credit_risks_analysis.html)

---

### 💰 [¿Quién gestiona mejor sus finanzas públicas?](./An%C3%A1lisis%20fiscal%20global/)
**Python · Pandas · Requests · Matplotlib · Seaborn · Tableau · Flourish · API del FMI**

Análisis de 30 años de datos fiscales (1995–2024) para 182 países. Construcción de tres métricas propias (índice de salud fiscal, dinámica fiscal, score de vulnerabilidad) y dashboard interactivo con linking & brushing entre mapa, scatter y matriz de riesgo. Hallazgo central: la riqueza de un país no predice su salud fiscal — seis economías de renta alta están en situación crítica en 2024, y ningún país del G7 domina el ranking histórico.

🔗 [Mapa y matriz de riesgo →](https://public.tableau.com/views/ViszQ1Q2yQ3/Anlisisfiscalglobal) · [Austeridad vs. desempleo →](https://public.tableau.com/views/ViszQ1Q2yQ3/AusteridadvsDesempleo) · [Ranking animado →](https://public.flourish.studio/visualisation/293399)

---

### 🚗 [Segmentación de riesgo y predicción de siniestralidad en seguros de vehículos](./insurance-risk-prediction/)
**R · K-means · DBSCAN · rpart · Random Forest · PCA/SVD**

Segmentación no supervisada (K-means, K-medians, PAM, DBSCAN) y predicción supervisada de siniestralidad sobre una cartera real de seguros de vehículos de motor, con reducción de dimensionalidad vía PCA/SVD. El árbol de decisión podado, entrenado de forma independiente, converge en las mismas variables que definen los clusters de mayor riesgo (validación cruzada entre ambos enfoques). Hallazgo central: las 287 pólizas marcadas como "ruido" por DBSCAN presentan la siniestralidad más alta de toda la cartera (32.4%), muy por encima de cualquier cluster principal.

🔗 [Ver análisis completo →](https://github.com/Canmor115/data-projects/blob/8d859d3b62d716e9a5b765332869c4afe1fffaa0/insurance-risk-prediction/02_clustering_classification.html)

---

## 🛠️ Stack tecnológico

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

---

## 👩‍💻 Autora

**Camila Moreno Ortiz**  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Camila%20Moreno-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/camila-andrea-moreno-ortiz/)
